...
const SLOTS = [
  ...
  { x: 0.88, y: 0.20, align: 'right' },
  { x: 0.10, y: 0.78, align: 'left'  },
  { x: 0.88, y: 0.80, align: 'right' },
  { x: 0.18, y: 0.50, align: 'left'  },
  { x: 0.82, y: 0.50, align: 'right' },
];
      ...
      setSlotState((prev) => {
        ...
        for (let i = 0; i < prev.length; i++) {
          ...
          if (now - s.startMs > s.lifeMs) {
            ...
            inUse.add(nextIdx);
            if (!dirty) { next = prev.slice(); dirty = true; }
            next[i] = {
              thoughtIdx: nextIdx,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {!isRight && <span style={{
          width: 4, height: 4, borderRadius: 2, background: ICE_ACCENT,
      ...
      <div style={{
        ...
        fontSize: 17, lineHeight: 1.3, fontWeight: 300,
